...
export default function Users() {
  ...
  async function createUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Registration returns HTTP 200 for duplicate emails as well as success.
      if (result.message === "Email already exists") {
        setFormError("An account with this email already exists.");
        return;
      }
      if (result.message !== "User registered") {
        setFormError("Unable to confirm user creation. Please refresh the user list before trying again.");
        return;
      }
      setName("");
      setEmail("");
  ...
  return <AdminPage title="Users" description="Create user accounts and view registered users and administrators.">
    <Card><Card.Content className="p-6">
      ...
      <p className="mb-5 mt-2 text-slate-600">New accounts have the user role and can log in to take quizzes.</p>
    ...
    <DataState {...request}>
      <Card><Card.Content className="overflow-x-auto">
        <table className="w-full border-collapse text-left">
          ...
          <thead><tr className="border-b"><th scope="col" className="p-4">Name</th><th scope="col" className="p-4">Email</th><th scope="col" className="p-4">Role</th><th scope="col" className="p-4">Actions</th></tr></thead>
          ...
          {users.length === 0 && <tr><td colSpan={4} className="p-8 text-center text-gray-500">{request.data?.length ? "No users match your search." : "No users registered yet."}</td></tr>}</tbody>
        </table>
      </Card.Content></Card>
    </DataState>
  </AdminPage>;
